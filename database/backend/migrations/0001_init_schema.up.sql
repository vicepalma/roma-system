...
CREATE INDEX idx_md_master   ON master_disciple(master_id);
CREATE INDEX idx_md_disciple ON master_disciple(disciple_id);

-- 2) Biblioteca: ejercicios y métodos
CREATE TABLE exercises (
  id              UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  name            TEXT NOT NULL,
  primary_muscle  TEXT NOT NULL,   -- chest|back|legs|shoulders|arms|core|...
  equipment       TEXT NULL,       -- barbell|dumbbell|machine|band|bodyweight|...
  tags            TEXT[] NOT NULL DEFAULT '{}',
  notes           TEXT NULL
);
CREATE UNIQUE INDEX uq_exercise_name ON exercises (lower(name));

CREATE TABLE methods (
  id      UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  key     TEXT NOT NULL UNIQUE,     -- 'fst7', '531', 'phul', ...
  name    TEXT NOT NULL,
  params  JSONB NOT NULL DEFAULT '{}' -- {series, rest_sec, to_failure, target_reps}
);
...
CREATE TABLE programs (
  ...
  title       TEXT NOT NULL,
  notes       TEXT NULL,
  visibility  TEXT NOT NULL DEFAULT 'private', -- private|unlisted|public (futuro)
...
CREATE INDEX idx_presc_day      ON prescriptions(day_id);
CREATE INDEX idx_presc_exercise ON prescriptions(exercise_id);

-- 4) Asignaciones (programa → discípulo) con versión fijada
...
CREATE INDEX idx_sets_session ON set_logs(session_id);

-- 6) Cardio (prescripción y realizado)
CREATE TABLE cardio_segments (
  ...
  modality      TEXT NOT NULL,   -- trote|bici|eliptica|remo|...
  minutes       INT  NOT NULL CHECK (minutes > 0),
  target_hr_min INT  NULL,
  target_hr_max INT  NULL,
  notes         TEXT NULL,
  CHECK ( (day_id IS NOT NULL) <> (session_id IS NOT NULL) ) -- XOR: uno u otro
);
CREATE INDEX idx_cardio_day     ON cardio_segments(day_id);
CREATE INDEX idx_cardio_session ON cardio_segments(session_id);
...
CREATE INDEX idx_checkins_disciple ON checkins(disciple_id);

-- 8) Flags del usuario (lesiones, equipo, nivel)
CREATE TABLE user_flags (
  id        UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id   UUID NOT NULL UNIQUE REFERENCES users(id) ON DELETE CASCADE,
  injuries  JSONB NOT NULL DEFAULT '{}'::jsonb,   -- {"hombro":true}
  equipment JSONB NOT NULL DEFAULT '{}'::jsonb,   -- {"mancuernas":true}
...
-- (opcional) marcar históricas como cerradas
UPDATE session_logs SET status = 'closed'
WHERE status = 'open';

-- (opcional) índice útil
CREATE INDEX IF NOT EXISTS idx_session_assign_day_status
  ON session_logs (assignment_id, day_id, status);
